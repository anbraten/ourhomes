  ...
  <div class="space-y-3">
    ...
    <div v-if="!compact" class="space-y-2">
      <div class="flex items-center justify-between text-sm">
        ...
        <div class="flex items-center">
          <img :src="getPaidByUser()?.avatar" :alt="getPaidByUser()?.name" class="w-6 h-6 rounded-full mr-2" />
          <span class="font-medium text-gray-900">{{ getPaidByUser()?.name }}</span>
        </div>
      </div>
      ...
      <div class="text-sm">
        ...
        <div class="flex -space-x-1 mt-1">
          <img
            v-for="participantId in item.data.participants"
            :key="participantId"
            :src="getParticipantUser(participantId)?.avatar"
            :alt="getParticipantUser(participantId)?.name"
            class="w-6 h-6 rounded-full border-2 border-white"
            :title="getParticipantUser(participantId)?.name"
          />
        </div>
      </div>
    </div>
...
const { getUsers } = useHome();
const users = getUsers();

const getPaidByUser = () => {
  return users.find((user) => user.id === props.item.data.paidBy);
};

const getParticipantUser = (id) => {
