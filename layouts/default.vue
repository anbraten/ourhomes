<template>
  <div class="min-h-screen bg-gray-50">
    ...
    <nav class="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 md:hidden">
      <div v-if="$route.params.homeId" class="flex justify-around py-2">
        <NuxtLink
          :to="`/homes/${$route.params.homeId}`"
          class="flex flex-col items-center py-2 px-3 text-xs transition-colors duration-200"
          :class="$route.path === `/homes/${$route.params.homeId}` ? 'text-primary-600' : 'text-gray-600'"
        >
        ...
        <NuxtLink
          :to="`/homes/${$route.params.homeId}/expenses`"
          class="flex flex-col items-center py-2 px-3 text-xs transition-colors duration-200"
          :class="$route.path === `/homes/${$route.params.homeId}/expenses` ? 'text-primary-600' : 'text-gray-600'"
        >
        ...
        <NuxtLink
          :to="`/homes/${$route.params.homeId}/settings`"
          class="flex flex-col items-center py-2 px-3 text-xs transition-colors duration-200"
          :class="$route.path === `/homes/${$route.params.homeId}/settings` ? 'text-primary-600' : 'text-gray-600'"
        >
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center h-16">
          <div class="flex items-center space-x-8">
            ...
            <div v-if="$route.params.homeId" class="flex space-x-4">
              <NuxtLink
                :to="`/homes/${$route.params.homeId}`"
                class="px-3 py-2 rounded-md text-sm font-medium transition-colors duration-200"
                :class="
                  $route.path === `/homes/${$route.params.homeId}`
              ...
              <NuxtLink
                ...
                class="px-3 py-2 rounded-md text-sm font-medium transition-colors duration-200"
                :class="
                  $route.path === `/homes/${$route.params.homeId}/expenses`
                    ? 'bg-primary-100 text-primary-700'
                    : 'text-gray-600 hover:text-gray-900'
                "
              >
              ...
              <NuxtLink
                ...
                class="px-3 py-2 rounded-md text-sm font-medium transition-colors duration-200"
                :class="
                  $route.path === `/homes/${$route.params.homeId}/settings`
                    ? 'bg-primary-100 text-primary-700'
                    : 'text-gray-600 hover:text-gray-900'
                "
              >
          ...
          <div class="flex items-center space-x-4" v-if="currentHome">
            ...
            <div class="flex -space-x-2">
              <img
                v-for="member in currentHome.members.slice(0, 3)"
                :key="member.id"
                :src="member.avatar"
                :alt="member.name"
              ...
              />
              <div
                v-if="currentHome.members.length > 3"
